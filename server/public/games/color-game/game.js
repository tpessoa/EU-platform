...
class playGame extends Phaser.Scene {
    ...
    async preload() {
        ...
        this.load.image(DB_image_ref_original, DB_src_painting);
    ...
    create() {
         ...
         * 
         * */
        var redo = this.add.image(0, 0, 'redo')
        ...
        */
        ...
        this.input.on('pointerdown',
            (pointer) => this.clickHandler(imgRef, pointer, imgOffsetX, imgOffsetY));
        ...
        SELECTEDCIRCLE = this.add.image(0, 0, 'circleImg')
            .setVisible(true)
        ...
        var arrow_left = this.add.image(0, 0, "carret")
            .setInteractive({ pixelPerfect: true })
            .setTintFill(0xffc425)
            .setPosition(redo.x, color_bar_y);
        var arrow_right = this.add.image(0, 0, "carret")
            .setInteractive({ pixelPerfect: true })
            .setTintFill(0xffc425)
            .setPosition(s_width - redo.x, color_bar_y)
            .setScale(-1, 1);

        arrow_left.on('pointerdown', (pointer) => this.getSwipedArr(pointer, 'right'));
        ...
        originalImg.on('pointerdown', (pointer) => this.createModal(bar_size, container_top_bar, originalImg));
    ...
    createModal(bar_size, container_top_bar, originalImg) {
        ...
        var transparent_rec = this.add.rectangle(0, 0, s_width, s_height, 0x000000)
            .setOrigin(0, 0)
            .setAlpha(0.8)
            .setInteractive();
        ...
        var closeBtn = this.add.image(0, 0, 'close')
            .setInteractive({ pixelPerfect: true })
            .setTintFill(0xffc425)
            .setOrigin(0, 0);
        closeBtn.setPosition(holder_rec_w - closeBtn.width - spaceBetweenIcons, spaceBetweenIcons);
        ...
        var name = this.make.text({
            ...
            style: {
                ...
                fontSize: font_size,
                color: '#4b86b4',
                wordWrap: { width: holder_rec_w - 3 * closeBtn.width, useAdvancedWrap: true }
            }
        });
        ...
        var holder_rec = this.add.rectangle(0, 0, holder_rec_w, originalImgModal.getBottomRight().y + side_gap, 0xffffff)
            .setOrigin(0, 0)
            .setAlpha(1)
            .setInteractive();
        ...
        closeBtn.on('pointerdown', () => this.modalHandler(modal_container, transparent_rec));
        transparent_rec.on('pointerdown', () => this.modalHandler(modal_container, transparent_rec));
    }
    ...
    redoHandler(pointer) {
        ...
        for (let i = 0; i < temp_arr.length; i++) {
            ...
            // SAFARI workarround
            ctx.fillStyle = 'rgb(' + temp_obj.color[0] + ',' + temp_obj.color[1] + ',' + temp_obj.color[2] + ')';
    ...
    * */
    createCircle(pos_x, pos_y, color) {
        ...
        var x_draw_circle = 0;
        if (pos_x == 0) {
            x_draw_circle = middle - (6 * smallerCircleRadius + 6 * spaceBetweenIcons);
        }
        if (pos_x === 1) {
            x_draw_circle = middle - (4 * smallerCircleRadius + 4 * spaceBetweenIcons);
        }
        else if (pos_x === 2) {
            x_draw_circle = middle - (2 * smallerCircleRadius + 2 * spaceBetweenIcons);
        }
        else if (pos_x === 3) {
    ...
    getSwipedArr(pointer, direction) {
        ...
        if (direction === 'right') {
            for (let i = 0; i < colorsCirclesObjs.length - 1; i++) {
                this.doSwipeAnimation(colorsCirclesObjs[i], colorsCirclesObjs[i + 1], ANIMATION_DURATION, direction);
                if (i === 0) {
    ...
    allAnimationsCompletedHandler(tweenRef, direction) {
        ...
        if (ANIMATIONS_COUNTER < colorsCirclesObjs.length - 2) {
            ...
            if (direction === 'right') {
                ...
                // update coordinates
                last_circle.x = game.scale.width / 2 - (6 * smallerCircleRadius + 6 * spaceBetweenIcons);
                ...
                //update coordinates
                first_circle.x = game.scale.width / 2 + (6 * smallerCircleRadius + 6 * spaceBetweenIcons);
            ...
            for (let i = 0; i < colorsCirclesObjs.length; i++) {
                if (SELECTEDCIRCLE.x === colorsCirclesObjs[i].x) {
                    NEW_COLOR_ARR = this.convertColorStrToArray(colorsCirclesObjs[i].getData('color'))
                }
            }
        }
    }
    ...
    doSwipeAnimation(targetObj, destObj, duration, direction) {
        var circleTween = this.tweens.add({
            ...
            ease: 'Power2',
            duration: duration,
            onComplete: () => this.allAnimationsCompletedHandler(circleTween, direction)
        })
    }
    ...
    clickHandler(imgObj, pointer, imgOffsetX, imgOffsetY) {
        ...
        if ((roundX >= imgObj.x && roundX <= (imgObj.width - imgObj.x))
            && roundY >= imgObj.y && roundY <= (imgObj.height - imgObj.y)) {
            ...
            if (startPixelData.r == NEW_COLOR_ARR[0] && startPixelData.g == NEW_COLOR_ARR[1] && startPixelData.b == NEW_COLOR_ARR[2]) {
                ...
                this.floodFill(roundX, roundY, startPixelData,
    ...
    floodFill(startX, startY, startPixelData, drawingAreaX, drawingAreaY, drawingAreaWidth, drawingAreaHeight) {
        ...
        while (pixelStack.length) {
            ...
            while (y >= drawingBoundTop && this.matchStartColor(startPixelData, pixelPos)) {
                y -= 1;
                pixelPos -= WIDTH * 4;
            }
            ...
            reachLeft = false;
            reachRight = false;
            while (y <= drawingBoundBottom && this.matchStartColor(startPixelData, pixelPos)) {
    ...
    matchStartColor(startPixelData, pixelPos) {
        ...
        if (r == startPixelData.r && g == startPixelData.g && b == startPixelData.b) {
            return true;
        }
    ...
    colorPixel(x, y, pixelPos) {
        ...
        let tempObj = {
            ...
            x: x,
            y: y,
            color: [data_arr.data[pixelPos], data_arr.data[pixelPos + 1], data_arr.data[pixelPos + 2]]
        };
        ...
        // SAFARI workarround
        ctx.fillStyle = 'rgb(' + NEW_COLOR_ARR[0] + ',' + NEW_COLOR_ARR[1] + ',' + NEW_COLOR_ARR[2] + ')';
