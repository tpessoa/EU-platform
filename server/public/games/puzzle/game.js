...
class Puzzle extends Phaser.Scene {
    ...
    create() {
        ...
        graphics.fillRoundedRect(0, -bar_size_height * 0.5, s_width, bar_size_height * 1.5, 32);
        graphics.setDepth(-2);
        ...
        var width_piece_rt, height_piece_rt,
            piece_draw_x, piece_draw_y;
        var info_piece;
        ...
        for (let j = 0; j < VERTICAL_PIECES; j++) {
            for (let i = 0; i < HORIZONTAL_PIECES; i++) {
                ...
                this.make.renderTexture({ width: width_piece_rt, height: height_piece_rt }, false)
                    .draw(puzzleImg_aux, -piece_draw_x, -piece_draw_y)
                    .saveTexture('piece[' + j + ',' + i + ']');

                var pieceImg_aux = this.add.image
                    (
                ...
                var pieceGraphic_aux = this.generateJigsawPiece(g_offsetX, g_offsetY, info_piece.t, info_piece.r, info_piece.b, info_piece.l, 1.5, 0x696969);
                pieceGraphic_aux
                    .setPosition(
                        puzzleImg.getTopLeft().x + piece_draw_x,
                        puzzleImg.getTopLeft().y + piece_draw_y
                ...
                pieceImg_aux.setData('lock_pos', { x: pieceGraphic_aux.x, y: pieceGraphic_aux.y })
                ...
                pieceGraphic_aux.setPosition(final_x + offset_piece_to_img_X, final_y + offset_piece_to_img_Y);
...
*/
        ...
        this.input.on('dragend', (pointer, gameObject) => this.dragEndHandler(gameObject));
    ...
    checkLockPosition(lock_pos, obj_x, obj_y) {
        ...
        if (obj_x > lock_pos.x - TOLERANCE_LOCK_OFFSET && obj_x < lock_pos.x + TOLERANCE_LOCK_OFFSET &&
            obj_y > lock_pos.y - TOLERANCE_LOCK_OFFSET && obj_y < lock_pos.y + TOLERANCE_LOCK_OFFSET) {
            return true;
        }
        return false;
    }
    ...
    generateBoard(puzzleImg) {
        ...
        for (let j = 0; j < VERTICAL_PIECES; j++) {
            for (let i = 0; i < HORIZONTAL_PIECES; i++) {
                ...
                graphic_piece = this.generateJigsawPiece(g_offsetX, g_offsetY, type_top, type_right, type_bottom, type_left, 1, 0x4b86b4);
                graphic_piece.setPosition(puzzle_offset.x + i * PIECE_W, puzzle_offset.y + j * PIECE_H);
    ...
    generateJigsawPiece(g_offsetX, g_offsetY, top, right, bottom, left, lineWidth, fillColorPiece) {
        ...
        var g_vec1, g_vec2,
            line1, line2;
        const startPoint = 4 / 10, endPoint = 6 / 10,
            controlPoint_1 = 3 / 10, controlPoint_2 = 5 / 10, controlPoint_3 = 7 / 10;
        ...
        if (top == 0) {
            ...
            g_vec1 = new Phaser.Math.Vector2(g_offsetX, g_offsetY);
            g_vec2 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * startPoint, g_offsetY);
            line1 = new Phaser.Curves.Line(g_vec1, g_vec2);
            path.add(line1);

            g_vec1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * endPoint, g_offsetY);
            g_vec2 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY);
            line2 = new Phaser.Curves.Line(g_vec1, g_vec2);
            path.add(line2);

            // the variable "top" permits reverse the socket bcz it multiplies by 1 or -1
            curve_p1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * startPoint, g_offsetY);
            curve_p2 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * controlPoint_1, g_offsetY - top * RADIUS_PIECE / 2);
            curve_p3 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * controlPoint_2, g_offsetY - top * RADIUS_PIECE);
            curve_p4 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * controlPoint_3, g_offsetY - top * RADIUS_PIECE / 2);
            curve_p5 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * endPoint, g_offsetY);

            curve = new Phaser.Curves.Spline([curve_p1, curve_p2, curve_p3, curve_p4, curve_p5]);
            path.add(curve);
        }

        if (right == 0) {
            g_vec1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY);
            g_vec2 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY + PIECE_H);
            ...
            g_vec1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY);
            g_vec2 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY + PIECE_H * startPoint);
            line1 = new Phaser.Curves.Line(g_vec1, g_vec2);
            path.add(line1);

            curve_p1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY + PIECE_H * startPoint);
            curve_p2 = new Phaser.Math.Vector2(g_offsetX + PIECE_W + right * RADIUS_PIECE / 2, g_offsetY + PIECE_H * controlPoint_1);
            curve_p3 = new Phaser.Math.Vector2(g_offsetX + PIECE_W + right * RADIUS_PIECE, g_offsetY + PIECE_H * controlPoint_2);
            curve_p4 = new Phaser.Math.Vector2(g_offsetX + PIECE_W + right * RADIUS_PIECE / 2, g_offsetY + PIECE_H * controlPoint_3);
            curve_p5 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY + PIECE_H * endPoint);

            curve = new Phaser.Curves.Spline([curve_p1, curve_p2, curve_p3, curve_p4, curve_p5]);
            path.add(curve);

            g_vec1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY + PIECE_H * endPoint);
            g_vec2 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY + PIECE_H);
            line1 = new Phaser.Curves.Line(g_vec1, g_vec2);
            path.add(line1);
        }
        ...
        if (bottom == 0) {
            g_vec1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY + PIECE_H);
            ...
            g_vec1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W, g_offsetY + PIECE_H);
            g_vec2 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * endPoint, g_offsetY + PIECE_H);
            line1 = new Phaser.Curves.Line(g_vec1, g_vec2);
            path.add(line1);

            curve_p1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * endPoint, g_offsetY + PIECE_H);
            curve_p2 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * controlPoint_3, g_offsetY + PIECE_H + bottom * RADIUS_PIECE / 2);
            curve_p3 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * controlPoint_2, g_offsetY + PIECE_H + bottom * RADIUS_PIECE);
            curve_p4 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * controlPoint_1, g_offsetY + PIECE_H + bottom * RADIUS_PIECE / 2);
            curve_p5 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * startPoint, g_offsetY + PIECE_H);

            curve = new Phaser.Curves.Spline([curve_p1, curve_p2, curve_p3, curve_p4, curve_p5]);
            path.add(curve);

            g_vec1 = new Phaser.Math.Vector2(g_offsetX + PIECE_W * startPoint, g_offsetY + PIECE_H);
            g_vec2 = new Phaser.Math.Vector2(g_offsetX, g_offsetY + PIECE_H);
        ...
        if (left == 0) {
            ...
            g_vec1 = new Phaser.Math.Vector2(g_offsetX, g_offsetY + PIECE_H);
            g_vec2 = new Phaser.Math.Vector2(g_offsetX, g_offsetY + PIECE_H * endPoint);
            line1 = new Phaser.Curves.Line(g_vec1, g_vec2);
            path.add(line1);

            curve_p1 = new Phaser.Math.Vector2(g_offsetX, g_offsetY + PIECE_H * endPoint);
            curve_p2 = new Phaser.Math.Vector2(g_offsetX - left * RADIUS_PIECE / 2, g_offsetY + PIECE_H * controlPoint_3);
            curve_p3 = new Phaser.Math.Vector2(g_offsetX - left * RADIUS_PIECE, g_offsetY + PIECE_H * controlPoint_2);
            curve_p4 = new Phaser.Math.Vector2(g_offsetX - left * RADIUS_PIECE / 2, g_offsetY + PIECE_H * controlPoint_1);
            curve_p5 = new Phaser.Math.Vector2(g_offsetX, g_offsetY + PIECE_H * startPoint);

            curve = new Phaser.Curves.Spline([curve_p1, curve_p2, curve_p3, curve_p4, curve_p5]);
            path.add(curve);

            g_vec1 = new Phaser.Math.Vector2(g_offsetX, g_offsetY + PIECE_H * startPoint);
            g_vec2 = new Phaser.Math.Vector2(g_offsetX, g_offsetY);
            line1 = new Phaser.Curves.Line(g_vec1, g_vec2);
            path.add(line1);
        }
    ...
    scaleImageToFitFrame(maxWidth, maxHeight, imgObj) {
        ...
        imgObj
            ...
            .setSize(Math.floor(imgObj.width * newScaleValue), Math.floor(imgObj.height * newScaleValue));
