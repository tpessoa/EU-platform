...
class WordSearch extends Phaser.Scene {
    ...
    createCostum() {
        ...
        var gameName = this.add.text(SIDE_GAP, Math.floor(bar_size_height / 2), "Sopa de Letras", { fontFamily: "Arial Black", fontSize: 32, color: "#4b86b4" });
         ...
         */
        var grid_container = this.visualWordSearch(grid_section, bar_size_height, main_puzzle_height);
        console.log('Words missing: ' + WORDS_INFO.not_placed);

        /**
         * WORDS SECTION
         */
        this.wordsSection(grid_section + SIDE_GAP, grid_container, WORDS_INFO.placed);

        /**
         * TIMER
         */
        if (time_flag) {
            var str_tempo = this.add.text(SIDE_GAP, Math.floor(bar_size_height / 2), "Tempo", { fontFamily: "Arial Black", fontSize: 24, color: "#4b86b4" });
            str_tempo.setStroke('#ffcc5c', 10);
            //  Apply the shadow to the Stroke and the Fill (this is the default)
            str_tempo.setShadow(2, 2, "#333333", 2, true, true);
            str_tempo.setOrigin(0, 0.5);
            str_tempo.setPosition(s_width - str_tempo.width - SIDE_GAP, Math.floor(bar_size_height / 4));

            timer_text = this.add.text(0, 0, { fontFamily: "Arial", fontSize: 30, color: "#4b86b4" });
            timer_text.setPosition(s_width - str_tempo.width / 2 - SIDE_GAP, Math.floor(bar_size_height / 2));

            timedEvent = this.time.delayedCall(MAX_TIME * mili, this.onEvent, [], this);
        }
    ...
    wordSearch(words_arr, words_arr_input) {
        ...
        var rnd_direction_pos, rnd_direction, info_direction, rnd_grid_pos, grid_pos;
        ...
        for (let i = 0; i < words_arr.length; i++) {
            ...
            while (word_poss_directions.length > 0 && !word_placed) {
                ...
                word_poss_directions = this.removeItemOnce(word_poss_directions, rnd_direction);
                ...
                while (grid_test_free_positions.length > 0 && !word_placed) {
                    ...
                    rnd_grid_pos = this.getRandomNum(0, grid_test_free_positions.length - 1);
                    ...
                    word_info.end_x = grid_pos.x + info_direction.right * (word_info.word.length - 1);
                    word_info.end_y = grid_pos.y + info_direction.down * (word_info.word.length - 1);
                    ...
                    grid_test_free_positions = this.removeItemOnce(grid_test_free_positions, grid_pos);
                }
            }
            if (!word_placed) {
    ...
    wordColision(grid, word_info) {
        ...
        for (let i = 0; i < word_info.word.length; i++) {
            ...
            if (current_char != GRID_EMPTY_CHAR && current_char != word_info.word[i]) {
                flagOverlap = true;
            }
        }
        return flagOverlap;
    ...
    visualWordSearch(s_width, bar_size_height, main_puzzle_height) {
        var cell_text = this.add.text(0, 0, '*', { fontFamily: "Arial", fontSize: CELL_SIZE / 1.5, color: 0x696969, align: 'center' }).setVisible(false);
        ...
        var grid_w_counter = 0, grid_h_counter = 0;

        for (let i = 0; i < HEIGHT; i++) {
            grid_w_counter = 0;
            for (let j = 0; j < WIDTH; j++) {
                var rt = this.add.renderTexture(0, 0, CELL_SIZE, CELL_SIZE).setVisible(false);
                ...
                var cell_img = this.add.image(grid_w_counter, grid_h_counter, cell_texture_name);
                ...
                cell_img.on('pointerdown', function (pointer) {
                    FIRST_CELL = { x: this.getData('grid_x'), y: this.getData('grid_y'), obj: this };
                });

                cell_img.on('pointerup', function (pointer) {
                    LAST_CELL = { x: this.getData('grid_x'), y: this.getData('grid_y'), obj: this };
                });
        ...
        grid_container.setPosition(s_width / 2 - grid_w_counter / 2, bar_size_height + main_puzzle_height / 2 - grid_h_counter / 2);
        ...
        var word_strikethrough = this.add.graphics({ lineStyle: { width: 3, color: yellow, alpha: 0.6 } });

        this.input.on('pointerdown', (pointer) => this.lineOriginHandler(pointer, line, SELECTION, grid_container));
        this.input.on('pointermove', (pointer) => this.lineMovingHandler(pointer, SELECTION, line, grid_container));
        this.input.on('pointerup', () => this.lineEndHandler(SELECTION, grid_container, word_strikethrough));
    ...
    lineMovingHandler(pointer, graphics, line, grid_container) {
        if (pointer.isDown && FLAG_INSIDE_GRID && this.inputInsideGrid(pointer, grid_container, true)) {
            // limit inside grid
            line.x2 = pointer.x;
            line.y2 = pointer.y;
    ...
    lineEndHandler(graphics, container, word_graphics) {
        ...
        if (FIRST_CELL.x == LAST_CELL.x) {
            ...
            for (let i = guess.frist_cell_pos; i <= guess.last_cell_pos; i += cell_offset) {
        ...
        // LEFT-DOWN
            ...
            for (let i = guess.frist_cell_pos; i <= left_down_offset_last_cell_pos; i += cell_offset) {
                // left-down position
        ...
        if (this.verifyWordGuess(guess)) {
            ...
            for (let i = 0; i < WORDS_INFO.w_section.length; i++) {
                ...
                if (guess.word == this.joinSpacedWord(obj._text)) {
                    ...
                    let line = new Phaser.Geom.Line(0, obj.y + obj.height / 2, obj.width, obj.y + obj.height / 2);
    ...
    verifyWordGuess(guess) {
        ...
        for (let i = 0; i < arr.length; i++) {
            if (arr[i].dir.type == guess.direction) {
                ...
                // reverse word
                else if (arr[i].word == this.reverseString(guess.word)) flag_right_guess = true;
            }
        }
        if (flag_right_guess) {
            // remove word, etc, etc
    ...
    wordsSection(start_x, grid_container, grid_words) {
        ...
        WORDS_INFO.w_container = this.add.container(Math.floor(start_x), Math.floor(grid_container.y));
        ...
        for (let i = 0; i < grid_words_arr.length; i++) {
            let word_in_words_container = this.add.text(0, word_height_container_counter, grid_words_arr[i].word_original,
                { fontFamily: "Arial", fontSize: Math.floor(CELL_SIZE / 2), color: '#FFFFFF' })
    ...
    inputInsideGrid(pointer, container, moving) {
        if (pointer.x >= container.x && pointer.x <= (container.x + container.getData('container_w'))
            && pointer.y >= container.y && pointer.y <= (container.y + container.getData('container_h'))) {
    ...
    getRandomProb(length) {
        ...
        // 4 directions
        if (length == 4) {
            if (num < 0.40) return 0;  //probability 0.40
            else if (num < 0.80) return 1; // probability 0.40
            else if (num < 0.90) return 2; //probability 0.10
            else return 3;  //probability 0.10
        }
        // 3 directions
        else if (length == 3) {
            if (num < 0.45) return 0;  //probability 0.45
            else if (num < 0.90) return 1; // probability 0.45
            else return 2; //probability 0.10
        }
        // 2 directions
        else if (length == 2) {
            if (num < 0.5) return 0;  //probability 0.5
