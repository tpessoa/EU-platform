  ...
  input: [
    {
      ...
      answers: [
        "Resposta 1",
        "Resposta 2",
        "Resposta 3 sdfsdfsd sdfsdf sdsdffewe2 ew fwef ef 2 f F S FD",
        "Resposta 4",
      ],
...
class Quiz extends Phaser.Scene {
  ...
  createCostum() {
    ...
    btn_next.setOrigin(0).setAlpha(0.5).disableInteractive();

    var question_container = this.createQuestionContainer(
      s_width,
      s_height,
      btn_next
    );
    answers_container = this.createAnswersContainer(
      s_width,
      s_height,
      question_container,
      btn_next
    );
    var ending_container = this.createEndingContainer(
      s_width,
      s_height,
      question_container,
      answers_container
    ...
    var current_answer_text =
      INFO.current_question_num + 1 + "/" + this.totalQuestions();
    CURRENT_ANSWER = this.add.text(
      question_container.x + question_container.dimensions.width / 2,
      question_container.y - 50,
      current_answer_text,
    ...
    q_marks = this.scaleImageToFitFrame(
      MAX_VAL,
      question_container.dimensions.height / 2.5,
      q_marks
    );
    q_marks.setDepth(-1).setOrigin(0);
    q_marks.setPosition(
      question_container.dimensions.width - q_marks.getData("scaled_w") / 2,
      question_container.dimensions.y - q_marks.getData("scaled_h") * 1.5
    );
    question_container.add(q_marks);
    btn_next.setPosition(
      question_container.dimensions.x + question_container.dimensions.width,
      answers_container.y + answers_container.height
    );
    ...
    if (INFO.current_question_num < INFO.input.length - 1) {
      ...
      if (INFO.timer) {
        ...
        timedEvent = this.time.delayedCall(
          INFO.time_to_resp_question * mili,
          this.onEvent,
          [s_width, s_height, q_container, btn_ref],
          this
        );
      }
    } else {
  ...
  update() {
    if (INFO.timer) {
      let current_time = (
        INFO.time_to_resp_question -
        timedEvent.elapsed / mili
      ).toFixed(0);
  ...
  createQuestionContainerWithTimer(
    s_width,
    s_height,
    q_container,
    text,
    ...
    timer_text = this.add.text(0, 0, "sfd", {
      fontFamily: "Arial",
      fontSize: 30,
      color: blue_text,
    });
    ...
    timedEvent = this.time.delayedCall(
      INFO.time_to_resp_question * mili,
      this.onEvent,
      [s_width, s_height, q_container, btn_next],
      this
    );
  ...
  createQuestionContainer(s_width, s_height, btn_next) {
    ...
    var text = this.make.text({
      ...
      style: {
        ...
        color: 0x696969,
        wordWrap: {
          width: questions_w - 2 * text_margin_w,
    ...
    if (INFO.timer) {
      return this.createQuestionContainerWithTimer(
        s_width,
        s_height,
        container,
        text,
    ...
    if (INFO.has_audio) {
      ...
      btn_listening
        .setInteractive()
        .setPosition(
          questions_w - btn_listening.getData("scaled_w") / 1.3,
          text.y + text.height / 3
        );

      btn_listening.on("pointerdown", () =>
        this.playQuestionAudioHandler(btn_listening)
      );
  ...
  createAnswersContainer(s_width, s_height, q_container, btn_next) {
    ...
    var rt = this.add
      .renderTexture(0, 0, answers_w, answers_h)
      .setVisible(false);
    rt.draw(graphics);
    ...
    for (let i = 0; i < NUM_ANSWERS; i++) {
      // placeholder
      let img_answer_placeholder = this.add.image(
        0,
        answers_h_counter,
        "answer_placeholder"
      );
      ...
      let a_number = this.add.text(0, 0, a_number_text, {
        fontFamily: "Arial",
        fontSize: answers_h * 0.4,
        color: 0x696969,
        align: "center",
      });
      let placeholder_center =
        img_answer_placeholder.height / 2 - a_number.height / 2;
      ...
      if (a_text.text.width > answers_w - SPACE_BETWEEN_ANSWERS) {
        a_text.setPosition(
          ...
          img_answer_placeholder.height / 2
        );
      } else {
        a_text.setPosition(
          SPACE_BETWEEN_ANSWERS,
          img_answer_placeholder.height / 2 + answers_h_counter
        );
      }

      // right or wrong information
      var info_correct = this.add.image(0, 0, "correct");
      info_correct = this.scaleImageToFitFrame(
        MAX_VAL,
        answers_h / 2,
        info_correct
      );
      info_correct
        .setPosition(
          answers_w - info_correct.getData("scaled_w") / 1.5,
          img_answer_placeholder.height / 2 + answers_h_counter
        )
        .setVisible(false)
      ...
      info_wrong = this.scaleImageToFitFrame(
        MAX_VAL,
        answers_h / 2,
        info_wrong
      );
      info_wrong
        .setPosition(
          answers_w - info_wrong.getData("scaled_w") / 1.5,
          img_answer_placeholder.height / 2 + answers_h_counter
        )
        .setVisible(false)
        .setData("type", "answer_sign")
      ...
      // Answer Handler
      img_answer_placeholder.on("pointerdown", () =>
        this.answerHandler(
          container,
          img_answer_placeholder,
          s_width,
          s_height,
          q_container,
          btn_next
      ...
      img_answer_placeholder.on("pointerover", () =>
        this.imgPointerOverHandler(img_answer_placeholder, true)
      );
      img_answer_placeholder.on("pointerout", () =>
        this.imgPointerOverHandler(img_answer_placeholder, false)
      );

      var arr_obj_in_placeholder = [
        img_answer_placeholder,
        a_number,
        a_text,
        info_correct,
    ...
    container.setPosition(
      s_width / 2 - answers_w / 2,
      q_container.dimensions.BR.y + 3 * SPACE_BETWEEN_ANSWERS
    );
    container.dimensions = {
      answer_w: answers_w,
      answer_h: answers_h,
  ...
  createEndingContainer(s_width, s_height, q_container, a_container) {
    ...
    var info_correct_pos_x =
      questions_w / 2 - info_correct.getData("scaled_w") * 2;
    info_correct
    ...
    var right_a_num = this.add.text(
      info_correct_pos_x,
      info_correct.y * 2,
      "1",
      { fontFamily: "Arial", fontSize: 50, color: 0x696969, align: "center" }
    );
    var wrong_a_num = this.add.text(info_wrong_pos_x, info_wrong.y * 2, "1", {
      fontFamily: "Arial",
      fontSize: 50,
      color: 0x696969,
      align: "center",
    ...
    var rect_summary = this.add.rectangle(
      0,
      0,
      rect_summary_w,
      rect_summary_h,
    ...
    rect_summary.on("pointerdown", () =>
      this.endingInfoHandler(
        s_width,
        s_height,
        q_container,
        a_container,
    ...
    var text = this.add.text(0, 0, "Ver Respostas", {
      fontFamily: "Arial",
      fontSize: 20,
      color: 0x696969,
      align: "center",
  ...
  answerHandler(
    ...
    // blur questions
    this.setBlurAndColorFromAnswers(
      container,
      white,
      0.4,
      false,
  ...
  setBlurAndColorFromAnswers(
    ...
    for (let i = 0; i < container.list.length; i++) {
      ...
      if (str != null && str.includes("img_placeholder")) {
        ...
        if (
          a_selected_pos != null &&
          a_selected_pos == list_obj.getData("answer_num")
        ) {
